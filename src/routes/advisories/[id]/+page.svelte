...
<script lang="ts">
  ...
  // The stable permalink for this revision: the canonical `/advisories/{id}` URL.
  // The detail page's own URL *is* that permalink, so we take the absolute path +
  // origin straight off `page.url` (correct behind any host/base-path). We avoid
  // `resolve("/advisories/[id]", …)` here because it yields a route-relative path
  // (`../advisories/1`), which is right for an in-page link but wrong for a
  // canonical/absolute permalink.
...
<svelte:head>
  <title>{t("detail.headTitle", { id: data.id })}</title>
  ...
  >
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div class="min-w-0">
        ...
        <h1 class="mt-2 text-xl font-bold break-words text-gray-900 sm:text-2xl dark:text-gray-100">
          {meta.title ?? meta.trackingId ?? `#${data.id}`}
        </h1>
        {#if meta.trackingId}
    ...
    >
      ...
      <div>
        ...
        <dd class="mt-0.5 break-words text-gray-900 dark:text-gray-100">{meta.category ?? "—"}</dd>
      </div>
      <div>
        <dt class="text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
          {t("detail.meta.version")}
        </dt>
        <dd class="mt-0.5 text-gray-900 tabular-nums dark:text-gray-100">{meta.version ?? "—"}</dd>
      </div>
      <div>
        <dt class="text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
          {t("detail.meta.cves")}

...
<script lang="ts">
  ...
  import { Status } from "$lib/CSAFWebview/docmodel/docmodeltypes";
  ...
  interface Props {
    // Kept for call-site compatibility with the upstream viewer. The portal has
    // no "related documents" navigation, so it is currently unused.
    basePath?: string;
  }
  let { basePath: _basePath }: Props = $props();
